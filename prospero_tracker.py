...
def get_current_price(ticker):
    try:
        # Fetching 1m data for the most recent price
...
def get_open_price(ticker, date_str):
    try:
        # Fetching historical data for the specific entry date
...
def main():
    ...
    if os.path.exists(CSV_FILE):
        ...
        # Added Current_Price to the columns
        df = pd.DataFrame(columns=['Ticker', 'Date_In', 'Price_In', 'Current_Price', 'Date_Out', 'Price_Out', 'Status', 'Days_Held', 'Gain_%'])
    ...
    for idx, row in df.iterrows():
        ticker = row['Ticker']
        date_in_str = row['Date_In']
        
        # Calculate Days Held
        date_in_dt = pd.to_datetime(date_in_str)
        ...
        # Update Live Data for Active Tickers
    ...
    # Add New Tickers (Using Opening Price)
    for ticker in current_tickers:
        if ticker not in df[df['Status'] == 'Active']['Ticker'].values:
            p_open = get_open_price(ticker, today_str)
            curr_p = get_current_price(ticker)
            
            # If the market hasn't opened yet today, p_open might be None; 
            # we fallback to current price in that rare case.
            final_in = p_open if p_open else curr_p
            
            if final_in:
                new_row = {
                    'Ticker': ticker, 'Date_In': today_str, 'Price_In': final_in, 
                    'Current_Price': curr_p, 'Status': 'Active', 'Days_Held': 0, 'Gain_%': '0.00%'
                }
